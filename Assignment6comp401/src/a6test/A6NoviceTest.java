...
public class A6NoviceTest {
	...
	static public String[] getTestNames() {
		...
		test_names[1] = "testGetSetPixelByCoord";
		test_names[2] = "testExtractSubpictureByCoord";
		test_names[3] = "testPixelIteratorBasic";
		test_names[4] = "testPixelIteratorSetterError";
		return test_names;
	}
	...
	public void testGetSetPixelByCoord() {
		...
		assertEquals("Pixel retrieved using coordinate after setting does not match expected value", RED,
				subpic_1.getPixel(sub_a));
		assertEquals("Pixel retrieved using x and y after setting does not match expected value", RED,
				subpic_1.getPixel(0, 0));
		assertEquals("Pixel in source not changed after setting through subpicture", RED, source_1.getPixel(source_a));
	}
	...
	public void testExtractSubpictureByCoord() {
		...
		assertEquals(
				"Pixel retrieved from subpicture using top-left/bottom-right coordinates does not match expected pixel value from source",
		...
		assertEquals(
				"Pixel retrieved from subpicture using top-right/bottom-left coordinates does not match expected pixel value from source",
	...
	public void testPixelIteratorBasic() {
		...
		assertEquals("The next pixel retrieved by the pixel iterator is not the expected pixel", RED,
				s1_iterator.next());
		// (1, 0)
		assertTrue(s1_iterator.hasNext());
		assertEquals("The next pixel retrieved by the pixel iterator is not the expected pixel", WHITE,
				s1_iterator.next());
		// (2, 0)
		assertTrue(s1_iterator.hasNext());
		assertEquals("The next pixel retrieved by the pixel iterator is not the expected pixel", GREEN,
				s1_iterator.next());
		// (3, 0)
		assertTrue(s1_iterator.hasNext());
		assertEquals("The next pixel retrieved by the pixel iterator is not the expected pixel", BLACK,
				s1_iterator.next());
		// (0, 1)
		assertTrue(s1_iterator.hasNext());
		assertEquals("The next pixel retrieved by the pixel iterator is not the expected pixel", GREEN,
		...
		// (2, 1)
		assertTrue(s1_iterator.hasNext());
		assertEquals("The next pixel retrieved by the pixel iterator is not the expected pixel", BLUE,
				s1_iterator.next());
		// (3, 1)
		assertTrue(s1_iterator.hasNext());
		assertEquals("The next pixel retrieved by the pixel iterator is not the expected pixel", RED,
